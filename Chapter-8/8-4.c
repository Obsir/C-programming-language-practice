...

{
    ...
    else
    {
        ...
        for (i = 0; i < OPEN_MAX; i++)
            ...
            if ((_iob[i].flag & (_WRITE | _ERR)) == _WRITE && _iob[i].base != NULL && _flush(&_iob[i]) == EOF)
...

{
    ...
    if ((fp->flag & _ERR) || (!(fp->flag & _READ) && !(fp->flag & _WRITE)))
    ...
    else
    {
        ...
        if ((newpos = lseek(fp->fd, 0, SEEK_END)) == -1 || lseek(fp->fd, fp->bufpos, SEEK_SET) == -1)
        {
